    ...
    <xsl:template match="/">
        <html>
            ...
            <body>
                ...
                <!-- End Site Header -->
                <div class="orgs-body">
                    <h1> The Organizations of <em><xsl:value-of select="//tei:title[@type='main']"
                             /></em>
                    </h1>
            ...
            <tbody>
                <xsl:for-each select="//tei:org">
                    ...
                    <tr>
                        ...
                        <td>
                            <p>
                                <xsl:value-of select="count(//tei:orgName[@ref=$xmlid])" />
                            </p>
                        </td>
                        <td>
                            <p>
                            <xsl:for-each select="//tei:orgName">
                                    <xsl:variable name="orgNameNumber" select="position()" />
                                    <xsl:variable name="orgRef">
                                        <xsl:value-of select="@ref" />
                                    </xsl:variable>
                                    <xsl:if test="$xmlid=$orgRef">
                                        <div class="line indent">
                                            <xsl:element name="a">
                                                <xsl:attribute name="href">
                                                  <xsl:value-of
                                                  select="concat('iodineandgasoline.html#o', $orgNameNumber)"
                                                   />
                                                </xsl:attribute>
                                                <xsl:apply-templates /> (<xsl:value-of
                                                  select="$orgNameNumber" />) </xsl:element>
                                        </div>
                                    </xsl:if>
                            </xsl:for-each>
                            </p>
